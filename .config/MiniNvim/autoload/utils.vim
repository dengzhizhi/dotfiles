...
function! utils#Cabbrev(key, value) abort
  let k_len = 1 + len(a:key)
  execute printf('cnoreabbrev <expr> %s (getcmdtype() == ":" && (getcmdpos() <= %d \|\| (getcmdline()[0:4] == "''<,''>" && getcmdpos() <= %d + 5))) ? %s : %s',
        \ a:key, k_len, k_len, string(a:value), string(a:key))
endfunction
...
function! utils#Get_titlestr() abort
  ...
  if l:title_str == ''
    let l:title_str = bufname()
  endif
  if  match(l:title_str, '^\s*$') > -1
    let l:title_str = '[No Name]'
  endif
  if &buflisted
    let l:ftime = getftime(expand('%'))
    if l:ftime > 0
      let l:title_str = l:title_str . strftime('%Y-%m-%d %H:%M', l:ftime)
    endif
  endif
